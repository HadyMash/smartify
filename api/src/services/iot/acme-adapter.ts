...
public mapDevice(device: any): Device | undefined {
    try {
        ...
        if (!device.capabilities || device.capabilities.length === 0) {
          throw new Error('Device mapping failed: capabilities array must not be empty');
        }
  
        const d: Device = {
            id: device.id,
            capabilities: device.capabilities.map(this.mapCapability).filter(Boolean),
  ...
  public async setDeviceStates(
    ...
    try {
      ...
      for (const [id, state] of validUpdates) {
        ...
        if (readonlyAttempt) {
          throw new Error(`Cannot modify readonly capabilities for device ${id}`);
        }
      }

      // If all checks pass, update the states
     
      await Promise.all(validUpdates.map(([id, state]) => this.setDeviceState(id, state)));
      return;
    } catch (e) {
