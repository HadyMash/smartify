...
describe('AcmeIoTAdapter', () => {
  ...
  describe('healthCheck', () => {
    it('should return true when API is healthy', async () => {
      ...
      expect(mockedAxios.get).toHaveBeenCalledWith('https://api.acme.com/health');
    });
  ...
  describe('mapDevice', () => {
    ...
    it('should return undefined for invalid device format', () => {
      const adapter = new AcmeIoTAdapter();
      const mockDevice = { capabilities: [{ id: 'test', type: 'switch', readonly: false }] }; // Missing `id`
  
      expect(() => adapter.mapDevice(mockDevice)).toThrow("Device mapping failed: missing ID");
    });
  ...
  describe('mapDeviceWithState', () => {
    it('should correctly map a device with state', () => {
      ...
      jest.spyOn(deviceWithStateSchema, 'parse').mockImplementation((device) => device as any);
  ...
  describe('mapDeviceWithPartialState', () => {
    it('should correctly map a device with partial state', () => {
      // Mock schema validation
      jest.spyOn(deviceWithPartialStateSchema, 'parse').mockImplementation((device) => device as any);
    ...
    it('should discover and map devices successfully', async () => {
      mockedAxiosInstance.get.mockResolvedValueOnce({
        ...
        data: [
          ...
          {
            id: 'device-456',
            capabilities: [{ type: 'brightness', min: 0, max: 100, isReadOnly: false }]
          }
        ]
      });
  ...
  describe('pairDevices', () => {
    ...
    it('should pair devices by Device objects', async () => {
      ...
      const devices: Device[] = [
        { id: 'device-123', capabilities: [{ id: 'test', type: 'switch' }], source: deviceSourceSchema.enum.acme },
        { id: 'device-456', capabilities: [{ id: 'test', type: 'switch' }], source: deviceSourceSchema.enum.acme }
      ];
  ...
  describe('unpairDevices', () => {
    it('should unpair devices by ID array', async () => {
      ...
      expect(mockedAxiosInstance.delete).toHaveBeenCalledWith('/pair/device-123');
      expect(mockedAxiosInstance.delete).toHaveBeenCalledWith('/pair/device-456');
    });
    ...
    it('should unpair devices by Device objects', async () => {
      ...
      const adapter = new AcmeIoTAdapter();
      const devices: Device[] = [
        { id: 'device-123', capabilities: [{ id: 'test', type: 'switch', readonly: false }], source: deviceSourceSchema.enum.acme },
        { id: 'device-456', capabilities: [{ id: 'test', type: 'switch', readonly: false }], source: deviceSourceSchema.enum.acme }
      ];
    ...
    it('should handle errors during device unpairing', async () => {
      ...
      mockedAxiosInstance.delete.mockRejectedValue(new Error('Unpairing error'));
  ...
  describe('getDevice', () => {
    it('should get and map a single device', async () => {
      ...
      expect(mockedAxiosInstance.get).toHaveBeenCalledWith('/devices/device-123');
      expect(result).toBeDefined();
      expect(result?.id).toBe('device-123');
      expect(result?.state.on).toBe(true);
    ...
    it('should handle API errors when getting a device', async () => {
      ...
      mockedAxiosInstance.get.mockRejectedValueOnce(new Error('Get device error'));
  ...
  describe('getDevices', () => {
    ...
    it('should handle API errors when getting devices', async () => {
      ...
      mockedAxiosInstance.get.mockRejectedValueOnce(new Error('Get devices error'));
  ...
  describe('setDeviceState', () => {
    ...
    it('should throw error when trying to modify readonly capability', async () => {
      ...
      await expect(adapter.setDeviceState('device-456', { locked: false }))
    ...
    it('should throw error when device is not found', async () => {
      ...
      await expect(adapter.setDeviceState('device-789', { brightness: 80 }))
    ...
    it('should handle API errors gracefully', async () => {
      ...
      const mockDevice: DeviceWithState = {
        id: 'device-123',
        capabilities: [{ id: 'brightness', type: 'multiswitch', readonly: false }],
        state: { brightness: 50 },
        source: 'acme',
      };
      ...
      await expect(adapter.setDeviceState('device-123', { brightness: 90 }))
  ...
  describe('setDeviceStates', () => {
    it('should update multiple device states successfully', async () => {
      ...
      expect(adapter.setDeviceState).toHaveBeenCalledWith('device-1', { brightness: 80 });
      expect(adapter.setDeviceState).toHaveBeenCalledWith('device-2', { power: false });
    });
    ...
    it('should throw an error if trying to modify readonly capabilities', async () => {
      ...
      ).rejects.toThrow('Cannot modify readonly capabilities for device device-3');
    });
    ...
    it('should skip state updates for devices that do not exist', async () => {
      ...
      expect(adapter.setDeviceState).toHaveBeenCalledWith('device-5', { speed: 5 });
      expect(adapter.setDeviceState).not.toHaveBeenCalledWith('device-6', expect.anything());
    });
  });  
});